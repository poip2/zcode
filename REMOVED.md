...
| 类别 | mdhero | zcode v0.1 | zcode v0.4 | 备注 |
|---|---|---|---|---|
...
| Frontend utils | 4 个 | 0 个 | 2 个 | +files.ts, ai.ts |
| Rust 源文件 | 4 个 | 2 个 | 21 个 | +agent, model, provider, skills, sse, error, settings, providers/, tools/（agent pipeline） |
| Rust 命令 | 12 个 | 4 个 | 10 个 | +read_dir_tree, path_exists, create_markdown_file, create_folder, save_api_key, call_ai_provider |
...
| **源文件总计** | **~60 个** | **~12 个** | **~47 个** | 前端 20 + Rust 21 + 测试 6 |
...
### 10.18 `src/lib/tauri/ai.ts` — AI 前端接口 ★ v0.4 新增
...
**源文件总计：41 个**（前端 20 个 + Rust 21 个，不含配置/图标/zcode-mock.html/测试文件）
...
- **AI 后端配置 + 安全存储**（v0.4 重构）：API Key 通过 `keyring` crate 存入 OS keychain（macOS Keychain / Windows Credential Manager / Linux secret-service），`zcode-settings.json` 仅存脱敏版（如 `sk-5d70d***5c60`）；新模块 `settings.rs` 统一管理 keychain 读写/删除/脱敏/旧版迁移；`ai.ts` 前端封装接口；keychain 不可用时 best-effort 降级不阻塞；启动时自动扫描 config + data 双目录迁移旧版明文 `apiKey`
- **共享 Store 实例**：`sharedStore.ts` 单例模式，供 recents 和 pinnedFolder 共用 `zcode-recents.json`
- **AI Agent Pipeline** ★ v0.3：完整的 AI 编程代理（Agent Loop + Provider + Tools + Skills），从 pi-agent-rust 移植，支持 Anthropic 原生 API 和 OpenAI Chat Completions（兼容 20+ 提供商），7 个工具（read/bash/edit/write/grep/find/ls + 路径安全），技能系统（YAML frontmatter + XML system prompt 注入），纯 tokio 异步运行时，6 个测试覆盖端到端/mock/工具单元/keychain 安全
