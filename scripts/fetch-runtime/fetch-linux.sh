#!/usr/bin/env bash
# 仅供本地 WSL/Linux 开发调试用，不进 CI、不进发布包。幂等。
set -euo pipefail
...
if [ ! -f "$BIN_DIR/uv" ]; then
  echo "==> Downloading uv (x86_64-unknown-linux-gnu)"
  TMP_UV_DIR="$(mktemp -d)"
  curl -fL "https://github.com/astral-sh/uv/releases/latest/download/uv-x86_64-unknown-linux-gnu.tar.gz" -o "$TMP_UV_DIR/uv.tar.gz"
...
if [ ! -f "$BIN_DIR/bun" ]; then
  echo "==> Downloading bun (linux-x64)"
  TMP_BUN_DIR="$(mktemp -d)"
  curl -fL "https://github.com/oven-sh/bun/releases/latest/download/bun-linux-x64.zip" -o "$TMP_BUN_DIR/bun.zip"
  unzip -q "$TMP_BUN_DIR/bun.zip" -d "$TMP_BUN_DIR/extract"
  cp "$TMP_BUN_DIR/extract/bun-linux-x64/bun" "$BIN_DIR/bun"
  chmod +x "$BIN_DIR/bun"
  rm -rf "$TMP_BUN_DIR"
else
...
if [ ! -d "$PY_DIR" ]; then
  ...
  ASSET_URL=$(curl -fsSL "${AUTH_HEADER[@]}" "https://api.github.com/repos/astral-sh/python-build-standalone/releases/tags/${PBS_TAG}" \
    ...
    | grep "cpython-3.12" \
    | grep "x86_64-unknown-linux-gnu-install_only.tar.gz" \
