...
pub fn build_provider(
    ...
    if base_url.to_lowercase().contains("anthropic") {
        ...
        eprintln!("[zcode] build_provider: OPENAI branch, provider={name}, model={model}, url={url}");
...
mod tests {
    ...
    fn anthropic_deepseek_url() {
        let p = build_provider("", "claude-sonnet", DUMMY_KEY, "https://api.deepseek.com/anthropic")
            .expect("should build");
    ...
    fn anthropic_official_url() {
        let p = build_provider("", "claude-sonnet", DUMMY_KEY, "https://api.anthropic.com/v1/messages")
            .expect("should build");
        assert_eq!(p.name(), "anthropic");
        assert_eq!(p.api(), "anthropic-messages");
    }
    ...
    fn empty_base_url_fallsback_openai() {
        let p = build_provider("custom", "gpt-4o", DUMMY_KEY, "")
            .expect("should build");
    ...
    fn generic_url_uses_openai() {
        let p = build_provider("deepseek", "deepseek-chat", DUMMY_KEY, "https://api.deepseek.com/v1/chat/completions")
            .expect("should build");
    ...
    fn openrouter_uses_openai() {
        let p = build_provider("openrouter", "openai/gpt-4o", DUMMY_KEY, "https://openrouter.ai/api/v1/chat/completions")
            .expect("should build");
    ...
    fn normalize_anthropic_already_complete_untouched() {
        let p = build_provider("", "claude-sonnet", DUMMY_KEY, "https://api.deepseek.com/anthropic/v1/messages")
            .expect("should build");
        assert_eq!(p.api(), "anthropic-messages");
    }
    ...
    fn normalize_openai_already_complete_untouched() {
        let p = build_provider("openai", "gpt-4o", DUMMY_KEY, "https://api.openai.com/v1/chat/completions")
            .expect("should build");
        assert_eq!(p.api(), "openai-completions");
    }
    ...
    fn normalize_empty_url_returns_default() {
        let p = build_provider("openai", "gpt-4o", DUMMY_KEY, "")
            .expect("should build");
        assert_eq!(p.api(), "openai-completions");
    ...
    fn test_normalize_url_appends() {
        assert_eq!(normalize_url("https://api.x.com/anthropic", "/v1/messages"),
    ...
    fn test_normalize_url_already_complete() {
        assert_eq!(normalize_url("https://api.x.com/anthropic/v1/messages", "/v1/messages"),
            "https://api.x.com/anthropic/v1/messages");
    }
