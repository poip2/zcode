...
/// Input parameters for the edit tool — supports both single oldText/newText
/// and an edits[] array for multiple replacements.
...
struct EditInput {
    ...
    #[serde(default)]
    edits: Option<Vec<EditInputEntry>>,
}

#[derive(Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
struct EditInputEntry {
    old_text: String,
    new_text: String,
}
...
impl Tool for EditTool {
    ...
    async fn execute(
        ...
        let parsed: EditInput = serde_json::from_value(input)
            .map_err(|e| Error::validation(format!("Invalid edit input: {e}")))?;

        let absolute_path = resolve_path(&parsed.path, &self.cwd);
        let absolute_path = enforce_cwd_scope(&absolute_path, &self.cwd, "edit")?;

        // Build edits vector from either edits[] array or single oldText/newText
        let mut edits: Vec<(String, String)> = Vec::new();

        if let Some(edits_array) = parsed.edits {
            for entry in edits_array {
                if entry.old_text.is_empty() {
                    return Err(Error::validation("Each edit must have non-empty oldText"));
                }
                if entry.new_text.len() > WRITE_TOOL_MAX_BYTES {
                    return Err(Error::validation(format!(
                        "New text size exceeds maximum ({} > {WRITE_TOOL_MAX_BYTES})",
                        entry.new_text.len()
                    )));
                }
                edits.push((entry.old_text, entry.new_text));
            }
        } else {
            let old_text = parsed.old_text.unwrap_or_default();
            let new_text = parsed.new_text.unwrap_or_default();
        ...
        let meta = tokio::fs::metadata(&absolute_path).await.map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                Error::tool("edit", format!("File not found: {}", parsed.path))
            } else {
        ...
        if !meta.is_file() {
            return Err(Error::tool("edit", format!("Path is not a regular file: {}", parsed.path)));
        }
